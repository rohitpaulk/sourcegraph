...
var versionRe = lazyregexp.New(fmt.Sprintf(`\b%s\b`, regexp.QuoteMeta("12.6")))
...
func mainErr() error {
	// Run pg12.6 locally if it exists
...
func mainContainer() error {
	logger.Printf("Running PostgreSQL 12.6 in docker")
...
func startDocker() (commandPrefix []string, shutdown func(), _ error) {
	if err := exec.Command("docker", "image", "inspect", "postgres:12.6").Run(); err != nil {
		logger.Println("docker pull postgres12.6")
		pull := exec.Command("docker", "pull", "postgres:12.6")
		pull.Stdout = logger.Writer()
		pull.Stderr = logger.Writer()
		if err := pull.Run(); err != nil {
			return nil, nil, errors.Wrap(err, "docker pull postgres12.6")
		}
	...
	server := exec.Command("docker", "run", "--rm", "--name", containerName, "-e", "POSTGRES_HOST_AUTH_METHOD=trust", "-p", "5433:5432", "postgres:12.6")
