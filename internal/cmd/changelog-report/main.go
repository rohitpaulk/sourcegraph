...
func (cl Changelog) ToSlackMessage(cli *SlackClient, gcs *storage.Client) (*SlackMessage, error) {
	var merged Release
	for _, r := range cl {
	...
	section := func(name string, cs []Change) ([]SlackBlock, error) {
		...
		block := SlackBlock{
			Type: "section",
			Text: &SlackText{Type: "mrkdwn", Text: text.String()},
		}
		...
		if len(avatarURLs) > 0 {
			...
			if err != nil {
				...
				block.Accessory = &SlackAccessory{
					Type:     "image",
					ImageURL: imageURL,
					AltText:  "Group avatar",
				}
			}
		}

		return []SlackBlock{{Type: "divider"}, block}, nil
	}
	...
	for _, s := range []struct {
		...
		if len(s.Changes) > 0 {
			blocks, err := section(s.Name, s.Changes)
			if err != nil {
				return nil, err
			}
