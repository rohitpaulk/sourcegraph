...
-- lsif_data_references. It does not need to be precise, but needs to include all known schema versions
-- so far so that we re-trigger the progress check. This should update itself back to its true count
-- quickly.
INSERT INTO lsif_data_references_schema_versions SELECT DISTINCT dump_id AS dump_id, 1 AS min_schema_version, 2 AS max_schema_version FROM lsif_data_references;
